...
# Initialize tkinter window structure
root = tk.Tk()
root.withdraw()

# --- STEP 1: INTERACTIVE FILE SELECTION ---
# Select Book file
print("Select Book Excel File...")
book_file_path = filedialog.askopenfilename(
    title="Select Book Excel File",
    filetypes=[("Excel files", "*.xlsx *.xls")]
)

# Select Portal file
print("Select Portal Excel File...")
portal_file_path = filedialog.askopenfilename(
    title="Select Portal Excel File",
    filetypes=[("Excel files", "*.xlsx *.xls")]
)

# Read DataFrames using the dynamic paths selected by the user
book_df = pd.read_excel(book_file_path, skiprows=6)
book_df = book_df.iloc[:-1] # Remove total row at bottom if it exists

portal_df = pd.read_excel(portal_file_path, sheet_name="B2B", skiprows=5)
...
# %% --- WEIGHTED CASCADING SELECTION SYSTEM ---
# Calculate the Weighted Score (Max score = 7.0)
...
# %% --- NEW INTERACTIVE EXPORT SECTION ---
print("Select where to save your final reconciliation report...")

# Ask user for filename and storage location
output_filename = filedialog.asksaveasfilename(
    title="Save GST Reconciliation Report As",
    defaultextension=".xlsx",
    filetypes=[("Excel files", "*.xlsx")],
    initialfile="July_GST_Reconciliation_Report.xlsx"
)

# Only export if the user didn't cancel out of the window
if output_filename:
    ...
    print(f"✅ Export complete! Report saved successfully at: {output_filename}")
else:
